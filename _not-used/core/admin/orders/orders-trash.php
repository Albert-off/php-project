...
// Аuto delete all orders with order_state DELETED where auto_delete_date has expired.
$output = runDailyCleanup();

// Getting all DELETED orders from DB.
$softDeletedOrders = getAllSoftDeletedOrders();


// --======== DATA PAGINATION ========-- 
// 1. Total count of ARCHIVED orders.
$totalOrders = count($softDeletedOrders);
...
$currentOrders = array_slice($softDeletedOrders, $offset, $ordersPerPage);
...
<body>
    <?php if($output): ?>
        <input id="autoDeleteResponse" type="hidden" value="<?= htmlspecialchars($output, ENT_QUOTES, 'UTF-8'); ?>">
    <?php endif; ?>
        ...
        <section class="page-section">
            ...
            <div class="data-table" id="data-table">
                ...
                <section class="table__header">
                    <h1>Deleted Orders List [<?= $totalOrders ?>]</h1>
                        ...
                        <tbody>
                            ...
                            <?php 
                                ...
                                <tr id="order-<?= $order['id'] ?>">
                                    ...
                                    <td><button class="arc-btn" onclick="restoreSelectedOrder(<?= $order['id'] ?>)">Restore</button></td>
                                    <td><button class="del-btn" onclick="permanentlyDeleteSelectedOrder(<?= $order['id'] ?>, <?= $index - 1; ?>)">Delete</button></td>
                                    <td><?= $order['auto_delete_date'] ?></td>
                                </tr>
                            <?php endforeach; ?>
